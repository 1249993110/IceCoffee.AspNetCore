using IceCoffee.AspNetCore.Models;
using IceCoffee.DbCore.Dtos;
using Mapster;
using Microsoft.AspNetCore.Mvc;
using Microsoft.Extensions.DependencyInjection;

namespace IceCoffee.AspNetCore.Controllers
{
    public class ApiControllerBase : ControllerBase
    {
        protected virtual UserInfo UserInfo => HttpContext.RequestServices.GetService<UserInfo>() ?? new UserInfo(User.Claims);

        #region SucceededResult

        protected virtual Response SucceededResult()
        {
            return new Response()
            {
                Status = HttpStatus.OK
            };
        }

        protected virtual Response<TData> SucceededResult<TData>()
        {
            return new Response<TData>()
            {
                Status = HttpStatus.OK
            };
        }

        protected virtual Response<TData> SucceededResult<TData>(TData data)
        {
            return new Response<TData>()
            {
                Status = HttpStatus.OK,
                Data = data
            };
        }

        protected virtual Response<PaginationQueryResult<TItems>> PaginationQueryResult<TItems>(int total, IEnumerable<TItems> items)
        {
            return new Response<PaginationQueryResult<TItems>>()
            {
                Status = HttpStatus.OK,
                Data = new PaginationQueryResult<TItems>()
                { 
                    Items = items, 
                    Total = total 
                }
            };
        }

        protected virtual Response<PaginationQueryResult<TItems>> PaginationQueryResult<TItems>(PaginationQueryResult<TItems> model)
        {
            return new Response<PaginationQueryResult<TItems>>()
            {
                Status = HttpStatus.OK,
                Data = model
            };
        }

        protected virtual Response<PaginationQueryResult<TItems>> PaginationQueryResult<TItems>(PaginationResultDto dto)
        {
            return new Response<PaginationQueryResult<TItems>>()
            {
                Status = HttpStatus.OK,
                Data = new PaginationQueryResult<TItems>()
                {
                    Items = dto.Items.Adapt<IEnumerable<TItems>>(), 
                    Total = dto.Total
                }
            };
        }
        #endregion SucceededResult

        #region FailedResult

        protected virtual Response FailedResult(string message)
        {
            return new Response()
            {
                Status = HttpStatus.BadRequest,
                Error = new Error() 
                {
                    Message = message
                }
            };
        }

        protected virtual Response FailedResult(string message, string[] details)
        {
            return new Response()
            {
                Status = HttpStatus.BadRequest,
                Error = new Error()
                {
                    Message = message,
                    Details = details
                }
            };
        }

        #endregion FailedResult

        #region ForbiddenResult
        protected virtual Response ForbiddenResult()
        {
            return new Response()
            {
                Status = HttpStatus.Forbidden
            };
        }
        #endregion ForbiddenResult
    }
}